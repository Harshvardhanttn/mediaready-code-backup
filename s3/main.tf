resource "aws_s3_bucket" "datastore" {
	bucket = "${var.environment_name}-${var.s3_bucket}"
	tags = {
    "Name"        = "${var.project_name}-s3-${var.environment_name}"
...
resource "aws_iam_role" "instance" {
  name               = "${var.environment_name}-app_service-role"
  path               = "/system/"
...
resource "aws_iam_policy" "example" {
  name   = "${var.environment_name}-app_service-policy"
  path   = "/"
